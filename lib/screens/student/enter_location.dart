...
class ChangeLocationPage extends StatefulWidget {
  ...
  const ChangeLocationPage({super.key, required this.currentLocation});
  @override
...
late final _locationManager= LocationDataManager();
  ...
  void _submit() async {
    if (_formKey.currentState!.validate() && selectedLocation != null) {
      ...
      if (email == null) {
       ...
       if (widget.currentLocation == selectedLocation) {
 
      Navigator.pop(context); // Navigate back first
      
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('You are already in this location.')),
      );
      return;
    }
    ...
    try {
      ...
      // Navigate back before showing the snackbar
      Navigator.pop(context);
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => HomeStudent(email: email),
        ),
      );
      // Show snackbar on the home page
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(response)),
      );
      
    } catch (e) {
      Navigator.pop(context); // Still navigate back on error
      ScaffoldMessenger.of(context).showSnackBar(
